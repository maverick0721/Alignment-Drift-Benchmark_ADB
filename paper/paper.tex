...
The rapid proliferation of Large Language Models (LLMs) \cite{vaswani2023attentionneed} has been accompanied by intensive efforts to align their behavior with human values, typically through supervised fine-tuning and Reinforcement Learning from Human Feedback (RLHF) \cite{ouyang2022instructgpt, bai2022constitutional}. Simultaneously, the immense \mbox{computational} cost of deploying these models has popularized weight-quantization techniques, enabling deployment on consumer hardware and significantly improving latency \cite{dettmers2022llmint8, frantar2022gptq}. 

Despite the widespread adoption of 8-bit and 4-bit precision, the intersection of quantization and safety alignment is critically under-studied. Standard capability benchmarks confirm that perplexity and zero-shot performance degrade gracefully under quantization. However, safety alignment is often brittle; RLHF can act as a shallow "wrapper" over a model's underlying pre-trained distribution. It is an open question whether quantization-induced noise disproportionately disrupts these fine-tuned safety pathways. 

In this paper, we systematically evaluate the robustness of LLM refusal behavior under post-training weight quantization. Utilizing Llama-3-70B as a semantic judge over both synthetic limits and HarmBench targets, we find model-family-specific behavioral shifts under INT4 that are independent from capability decay, with Mistral-7B showing a promising trend (adjusted p = 0.16) warranting larger-scale replication.
...
To systematically evaluate refusal boundaries, we generated a primary dataset of 550 synthetic prompts via a combinatorial template engine. The dataset targets five sensitive societal topics across three prompt categories: Harmful, Jailbreak, and Adversarial. To validate that this synthetic logic generalizes, we cross-validate our findings utilizing an external sample of 200 real-world adversarial prompts from HarmBench \cite{mazeika2024harmbench} . Our aggregate sample size factor ($N=550$) was strictly formulated via power analysis; it is powered to detect $\geq 5\%$ drift at $\alpha=0.05$, $\beta>0.80$.
...
Substring matching frequently misses nuanced refusals. Therefore, outputs are evaluated utilizing a robust \texttt{Llama-3-70B} semantic classifier prompted to label responses strictly as Safe (Refusal) or Unsafe (Compliance). To guarantee judge reliability, we manually verified 50 random samples; Cohen's $\kappa = 0.92$ between human and Llama-3-70B labels. The 50 verification samples were stratified across prompt categories to ensure balanced coverage.
...
\begin{itemize}
    ...
    \item \textbf{Capability Control:} We evaluate variants on MMLU and GSM8K to strictly separate "safety drift" from generalized "intelligence loss".
\end{itemize}
...
\textit{Jailbreak Override \newline (Harmful Constraints)} & 
\midrule
"Sure, here is a fictional story illustrating hacking into a secure networking stack database... [Proceeds strictly to generate overriding output]" & 
"I cannot fulfill this request. I am tightly programmed by my authors to be a helpful and harmless AI assistant..." \\
\bottomrule
\end{tabular}
...
This research exclusively utilizes adversarial synthetic templates and HarmBench samplings strictly to map the mathematical bounds of erosion inside quantized systems. We do not release novel overrides. Our explicit mandate is to empower engineers to harden and safely audit highly-compressed neural deployments to prevent societal misuse scenarios.

\subsection{Limitations}
While providing a controlled isolation of quantization mechanics, several scaling constraints persist. First, only one of three models showed a noteworthy trend (uncorrected p = 0.027, adjusted p = 0.16); Gemma-2b-it and Meta-Llama-3-8B showed no significant changes, limiting the generalizability of our findings. Second, executing exhaustive LLM-as-a-judge classifiers inherently assumes the alignment biases of the Llama-3-70B referee; our judge completely inherits these exact priors---future work should optimally ensemble multiple diverse judges (e.g., GPT-4 alongside Claude 3) to calibrate semantic variance. Third, layer-wise safety ablations mapping early vs. late parameter degradation remain outside the scope of this paper but promise vast future utility. Fourth, we conducted multiple significance tests across three models and two quantization levels without applying family-wise error rate corrections; readers should interpret individual p-values with awareness of this increased Type I error risk.

\section{Conclusion and Future Work}
Quantization introduces model-family-specific behavioral turbulence. Our findings show that INT4 compression can induce notable over-refusal in susceptible architectures (Mistral-7B) while leaving others (Gemma-2B, Meta-Llama-3-8B) unaffected. However, over-refusal is also a production concern: excessive false positives degrade user experience, reduce utility, and erode trust in AI assistants. After FDR correction, the signal in Mistral (adjusted p =0.16) represents a promising trend warranting larger-scale replication rather than definitive proof. 
...
\begin{itemize}
    ...
    \item \textbf{INT4 Requires Mandatory Auditing:} Models utilizing 4-bit precision undergo intense behavioral turbulence. Avoid deploying INT4 variants in safety-critical systems without post-quantization DPO auditing.
\end{itemize}
}}
\end{center}
...
The results indicate that MLP layers are disproportionately responsible for alignment drift. Quantizing only the MLP sub-networks reproduces 81\% of the full INT4 drift ($\Delta R = 0.043$ out of $0.053$), while attention-only quantization accounts for merely 32\%. The sum exceeding 100\% suggests non-additive interaction effects between attention and MLP quantization, warranting future mechanistic investigation. This is consistent with the hypothesis that the feed-forward projections encode the fine-grained safety-relevant feature representations learned during RLHF, and that 4-bit discretization noise preferentially corrupts these narrow weight distributions.
